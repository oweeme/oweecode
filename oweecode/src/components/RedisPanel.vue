...
import { useRedisStore, type RedisConn } from '../composables/useRedisStore'
import { useI18n } from '../composables/useI18n'

const store = useEditorStore()
const { t } = useI18n()
const emit = defineEmits<{ 'open-conn': [connId: string, connName: string] }>()

const { connections, connectedIds, save } = useRedisStore()
const connectingId = ref<string | null>(null)
const errorMsg = ref('')
const showForm = ref(false)
const editingConn = ref<RedisConn | null>(null)

const form = ref<Omit<RedisConn, 'id'>>({
  name: '', host: 'localhost', port: 6379, password: '', db: 0,
})

function openAddForm() {
  editingConn.value = null
  form.value = { name: '', host: 'localhost', port: 6379, password: '', db: 0 }
  showForm.value = true
  errorMsg.value = ''
}

function openEditForm(c: RedisConn) {
  editingConn.value = c
  form.value = { ...c }
  showForm.value = true
  errorMsg.value = ''
...
function submitForm() {
  if (!form.value.host) { errorMsg.value = `${t('host')} (${t('required')})`; return }
...
function removeConn(id: string) {
  if (connectedIds.value.has(id)) invoke('redis_disconnect', { id })
  connectedIds.value.delete(id)
  connections.value = connections.value.filter(c => c.id !== id)
  save()
}

function buildUrl(c: RedisConn): string {
  const auth = c.password ? `:${encodeURIComponent(c.password)}@` : ''
  return `redis://${auth}${c.host}:${c.port}/${c.db}`
}

async function toggleConnect(c: RedisConn) {
  errorMsg.value = ''
  if (connectedIds.value.has(c.id)) {
    await invoke('redis_disconnect', { id: c.id })
  ...
  try {
    await invoke('redis_connect', { id: c.id, url: buildUrl(c) })
    connectedIds.value.add(c.id)
    connectedIds.value = new Set(connectedIds.value)
    emit('open-conn', c.id, c.name)
    store.openRedisConn(c.id, c.name)
  } catch (e: any) {
...
<template>
  <div class="ftp-panel">
    <div class="ftp-header">
      <span class="ftp-title">{{ t('redis') }}</span>
    ...
    <div class="ftp-list">
      ...
      >
        ...
        <div class="ftp-conn-info">
          ...
          <div class="ftp-conn-host">{{ c.host }}:{{ c.port }} · db{{ c.db }}</div>
        </div>
    ...
    <div v-if="showForm" class="ftp-form-overlay" @click.self="cancelForm">
      <div class="ftp-form">
        <div class="ftp-form-title">{{ editingConn ? t('editConnection') : t('newConnection') }} — Redis</div>

        <label>{{ t('name') }}</label>
        <input v-model="form.name" class="ftp-input" placeholder="My Redis" />

        <div class="ftp-row2">
          <div>
            <label>{{ t('host') }}</label>
            <input v-model="form.host" class="ftp-input" placeholder="localhost" />
          </div>
        ...
        <input v-model="form.password" class="ftp-input" type="password" placeholder="••••••••" />

        <label>{{ t('fieldDatabase') }} (index)</label>
        <input v-model.number="form.db" class="ftp-input" type="number" min="0" max="15" />
